...
usage:
	npx saddle script -n {network} flywheel:init {
		batch: number || null, // how many borrowrs to claim chum for at once. default 100 (~2.3M gas)
...
To test locally:
	1) ganache-cli --gasLimit 20000000 --gasPrice 20000 --defaultBalanceMATIC 1000000000 --allowUnlimitedContractSize true
		* use ^6.9
...
let isKnownNetwork = (src) => {
	return ['kovan', 'ropsten', 'goerli', 'mainnet', 'rinkeby', 'matic', 'bscmainnet'].includes(src);
};
...
let filterInitialized = async (borrowersByCToken) => {
	let res = {}
	let batchSize = 75;
	console.log(`Calling chumhumBorrowerIndex for borrowers in batches of ${batchSize}...\n`);
	for(let cTokenAddr of Object.keys(borrowersByCToken)) {
		let speed = await call(Comptroller, 'chumhumSpeeds', [cTokenAddr]);
		if (Number(speed) != 0){
			for (let borrowerChunk of getChunks(borrowersByCToken[cTokenAddr], batchSize)) {
				try {
					let indices = await Promise.all(borrowerChunk.map(
						async(borrower) => {
							return await call(Comptroller, 'chumhumBorrowerIndex',[cTokenAddr, borrower])
					}));
...
let claimChumHumBatch = async (borrowersByCToken, opts) => {
	for (let cTokenAddr of Object.keys(borrowersByCToken)) {
		...
		for (let chunk of getChunks(borrowers, opts.batch)) {
			if (chunk.length == 0) {
				...
				try {
					let tx = await send(Comptroller, 'claimChumHum', [
						chunk,
						[cTokenAddr],
						true,
						false,
...
(async () => {
	...
	await claimChumHumBatch(unInit, opts);
