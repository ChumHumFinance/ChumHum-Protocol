...
const { address, maticUnsigned } = require('./Utils/BSC');
const diff = require('jest-diff').default;
const { ComptrollerErr, BUMControllerErr, TokenErr, IRErr, MathErr } = require('./Errors');
...
expect.extend({
  ...
  toHaveBUMTrollError(actual, expectedErrorName) {
    return hasError.call(this, actual, expectedErrorName, BUMControllerErr);
  },
  ...
  toHaveBUMTrollFailure(result, err, info, detail=undefined) {
    return hasFailure.call(this, result, err, info, detail, BUMControllerErr, 'toHaveBUMTrollFailure');
  },
  ...
  toHaveBUMTrollMathFailure(result, info, detail) {
    return hasFailure.call(this, result, 'MATH_ERROR', info, detail && (MathErr.Error[detail] || -1), BUMControllerErr, 'toHaveBUMTrollMathFailure');
  },
  ...
  toHaveBUMTrollReject(result, info, detail) {
    return hasFailure.call(this, result, 'REJECTION', info, detail && ComptrollerErr.Error[detail], BUMControllerErr, 'toHaveBUMTrollReject');
  },

  toHaveBUMTrollErrorTuple(result, tuple, cmp=undefined) {
    return hasErrorTuple.call(this, result, tuple, BUMControllerErr, cmp);
  },

  toEqualNumber(actual, expected) {
    return match.call(this, actual, expected, maticUnsigned(actual).eq(maticUnsigned(expected)), opts('toEqualNumber'));
  },
