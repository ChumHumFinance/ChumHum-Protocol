...
async function bumBalance(bum, addr) {
  return ethers.utils.bigNumberify(new BigNum(await web3.eth.getBalance(addr)).toFixed());
}

async function maticGasCost(receipt) {
...
function maticExp(num) { return maticMantissa(num, 1e18) }
function maticDouble(num) { return maticMantissa(num, 1e36) }
function maticMantissa(num, scale = 1e18) {
  if (num < 0)
