const { address, both, maticMantissa } = require('../Utils/BSC');
const { makeComptroller, makeCToken } = require('../Utils/ChumHum');
    ...
    describe('succeeding', () => {
      ...
      it(`pauses Protocol`, async() => {
        ...
        globalMethods.forEach(async (method) => {
          switch (method) {
            case "Mint":
              await expect(send(comptroller, 'mintAllowed', [cToken._address, address(2), 1])).rejects.toRevert(`revert protocol is paused`);
              break;
  
            case "Borrow":
              await expect(send(comptroller, 'borrowAllowed', [cToken._address, address(2), 1])).rejects.toRevert(`revert protocol is paused`);
              break;
