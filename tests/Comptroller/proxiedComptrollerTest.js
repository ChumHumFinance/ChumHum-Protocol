const { address, maticMantissa } = require('../Utils/BSC');

const { makeComptroller, makeCToken, makePriceOracle } = require('../Utils/ChumHum');
...
describe('Comptroller', function() {
  ...
  let initializeBrains = async (priceOracle, closeFactor) => {
    ...
    await send(unitrollerAsBrain, '_setLiquidationIncentive', [maticMantissa(1)]);
    return unitrollerAsBrain;
  };

  describe('delegating to comptroller', () => {
    const closeFactor = maticMantissa(0.051);
    let unitrollerAsComptroller, cToken;

    beforeEach(async () => {
      unitrollerAsComptroller = await initializeBrains(oracle, maticMantissa(0.06));
