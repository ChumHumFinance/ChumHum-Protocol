...
const {fastForward, makeCToken} = require('../Utils/ChumHum');

const factor = maticMantissa(.02);

const reserves = maticUnsigned(3e12);
const cash = maticUnsigned(reserves.mul(2));
const reduction = maticUnsigned(2e12);
...
describe('CToken', function () {
  ...
  describe('_setReserveFactorFresh', () => {
    ...
    it("rejects newReserveFactor that descales to 1", async () => {
      expect(await send(cToken, 'harnessSetReserveFactorFresh', [maticMantissa(1.01)])).toHaveTokenFailure('BAD_INPUT', 'SET_RESERVE_FACTOR_BOUNDS_CHECK');
      expect(await call(cToken, 'reserveFactorMantissa')).toEqualNumber(0);
    });
  ...
  describe('_setReserveFactor', () => {
    ...
    it("returns error from setReserveFactorFresh without emitting any extra logs", async () => {
      const {reply, receipt} = await both(cToken, '_setReserveFactor', [maticMantissa(2)]);
  ...
  describe("_reduceReservesFresh", () => {
    ...
    it("fails if amount exceeds available cash", async () => {
      ...
      expect(await send(cToken, 'harnessReduceReservesFresh', [reserves])).toHaveTokenFailure('TOKEN_INSUFFICIENT_CASH', 'REDUCE_RESERVES_CASH_NOT_ABUMLABLE');
      expect(await call(cToken, 'totalReserves')).toEqualNumber(reserves);
    });

    it("increases admin balance and reduces reserves on success", async () => {
      const balance = maticUnsigned(await call(cToken.underlying, 'balanceOf', [root]));
