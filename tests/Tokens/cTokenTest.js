...
describe('CToken', function () {
  ...
  describe('borrowRatePerBlock', () => {
    it("has a borrow rate", async () => {
      ...
      const blocksPerYear = await call(cToken.interestRateModel, 'blocksPerYear');
  ...
  describe('supplyRatePerBlock', () => {
    ...
    it("has a supply rate", async () => {
      ...
      await send(cToken, 'harnessSetReserveFactorFresh', [maticMantissa(.01)]);
      await send(cToken, 'harnessExchangeRateDetails', [1, 1, 0]);
      await send(cToken, 'harnessSetExchangeRate', [maticMantissa(1)]);
  ...
  describe("borrowBalanceStored", () => {
    ...
    it("reverts on overflow of principal", async () => {
      await pretendBorrow(cToken, borrower, 1, 3, '0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF');
  ...
  describe('exchangeRateStored', () => {
    ...
    it("returns initial exchange rate with zero cTokenSupply", async () => {
      ...
      expect(result).toEqualNumber(maticMantissa(exchangeRate));
    });
    ...
    it("calculates with single cTokenSupply and single total borrow", async () => {
      ...
      const result = await call(cToken, 'exchangeRateStored');
      expect(result).toEqualNumber(maticMantissa(1));
    });

    it("calculates with cTokenSupply and total borrows", async () => {
      const cTokenSupply = 100e18, totalBorrows = 10e18, totalReserves = 0;
      await send(cToken, 'harnessExchangeRateDetails', [cTokenSupply, totalBorrows, totalReserves].map(maticUnsigned));
      const result = await call(cToken, 'exchangeRateStored');
      expect(result).toEqualNumber(maticMantissa(.1));
    });
    ...
    it("calculates with cash and cTokenSupply", async () => {
      ...
      expect(
        await send(cToken.underlying, 'transfer', [cToken._address, maticMantissa(500)])
      ).toSucceed();
      await send(cToken, 'harnessExchangeRateDetails', [cTokenSupply, totalBorrows, totalReserves].map(maticUnsigned));
      const result = await call(cToken, 'exchangeRateStored');
      expect(result).toEqualNumber(maticMantissa(100));
    });
    ...
    it("calculates with cash, borrows, reserves and cTokenSupply", async () => {
      ...
      await send(cToken, 'harnessExchangeRateDetails', [cTokenSupply, totalBorrows, totalReserves].map(maticUnsigned));
      const result = await call(cToken, 'exchangeRateStored');
      expect(result).toEqualNumber(maticMantissa(1.99));
    });
  });
