...
const borrowAmount = maticUnsigned(10e3);
...
describe('CToken', function () {
  ...
  describe('borrowFresh', () => {
    ...
    it("fails if error if protocol has less than borrowAmount of underlying", async () => {
      expect(await borrowFresh(cToken, borrower, borrowAmount.add(1))).toHaveTokenFailure('TOKEN_INSUFFICIENT_CASH', 'BORROW_CASH_NOT_ABUMLABLE');
    });
    ...
    it("fails if calculating account new total borrow balance overflows", async () => {
      await pretendBorrow(cToken, borrower, 1e-18, 1e-18, '0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF');
    ...
    it("fails if calculation of new total borrow balance overflows", async () => {
      await send(cToken, 'harnessSetTotalBorrows', ['0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF']);
    ...
    it("stores new borrow principal and interest index", async () => {
      ...
      expect(borrowSnap.interestIndex).toEqualNumber(maticMantissa(3));
      expect(await totalBorrows(cToken)).toEqualNumber(beforeProtocolBorrows.add(borrowAmount));
    });
  });
  ...
  describe('borrow', () => {
    ...
    it("returns error from borrowFresh without emitting any extra logs", async () => {
      expect(await borrow(cToken, borrower, borrowAmount.add(1))).toHaveTokenFailure('TOKEN_INSUFFICIENT_CASH', 'BORROW_CASH_NOT_ABUMLABLE');
    });
    ...
    [true, false].forEach((benefactorIsPayer) => {
      ...
      describe(label, () => {
        ...
        it("stores new borrow principal and interest index", async () => {
          ...
          expect(afterAccountBorrows.interestIndex).toEqualNumber(maticMantissa(1));
  ...
  describe('repayBorrow', () => {
    ...
    it("repays the full amount owed if payer has enough", async () => {
      await fastForward(cToken);
      expect(await repayBorrow(cToken, borrower, '0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF')).toSucceed();
      const afterAccountBorrowSnap = await borrowSnapshot(cToken, borrower);
    ...
    it("fails gracefully if payer does not have enough", async () => {
      ...
      await fastForward(cToken);
      await expect(repayBorrow(cToken, borrower, '0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF')).rejects.toRevert('revert Insufficient balance');
    });
  });
