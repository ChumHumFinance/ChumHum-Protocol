...
const redeemedAmount = redeemAmount.mul(maticUnsigned(9999e14)).div(maticUnsigned(1e18));
const feeAmount = redeemAmount.mul(maticUnsigned(1e14)).div(maticUnsigned(1e18));
...
describe('CToken', function () {
  ...
  beforeEach(async () => {
    ...
    cToken = await makeCToken({comptrollerOpts: {kind: 'boolFee'}, exchangeRate});
  });
    ...
    describe(redeemFresh.name, () => {
      ...
      it("fails if exchange calculation fails", async () => {
        if (redeemFresh == redeemFreshTokens) {
          expect(await send(cToken, 'harnessSetExchangeRate', ['0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF'])).toSucceed();
      ...
      it("transfers the underlying cash, tokens, and emits Redeem, Transfer events", async () => {
        ...
        expect(afterBalances).toEqual(await adjustBalances(beforeBalances, [
          [cToken, redeemer, 'cash', redeemedAmount],
  ...
  describe('redeem', () => {
    ...
    it("returns success from redeemFresh and redeems the right amount", async () => {
      ...
      expect(await balanceOf(cToken.underlying, redeemer)).toEqualNumber(redeemedAmount);
    });
    ...
    it("returns success from redeemFresh and redeems the right amount of underlying", async () => {
      ...
      expect(redeemAmount).not.toEqualNumber(0);
      expect(await balanceOf(cToken.underlying, redeemer)).toEqualNumber(redeemedAmount);
      expect(await balanceOf(cToken.underlying, devFee)).toEqualNumber(feeAmount);
    });
