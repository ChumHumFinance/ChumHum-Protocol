...
describe('GovernorAlpha#queue/1', () => {
  ...
  describe("overlapping actions", () => {
    it("reverts on queueing overlapping actions in same proposal", async () => {
      ...
      const chum = await deploy('CHUM', [root]);
      const gov = await deploy('GovernorAlpha', [timelock._address, chum._address, root]);
      const txAdmin = await send(timelock, 'harnessSetAdmin', [gov._address]);

      await enfranchise(chum, a1, 3e6);
      await mineBlock();

      const targets = [chum._address, chum._address];
      const values = ["0", "0"];
    ...
    it("reverts on queueing overlapping actions in different proposals, works if waiting", async () => {
      ...
      await enfranchise(chum, a1, 3e6);
      await enfranchise(chum, a2, 3e6);
      await mineBlock();

      const targets = [chum._address];
      const values = ["0"];
