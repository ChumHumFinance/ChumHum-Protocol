...
async function enfranchise(chum, actor, amount) {
  await send(chum, 'transfer', [actor, maticMantissa(amount)]);
  await send(chum, 'delegate', [actor], { from: actor });
}

describe("governorAlpha#castVote/2", () => {
  let chum, gov, root, a1, accounts;
  let targets, values, signatures, callDatas, proposalId;

  beforeAll(async () => {
    [root, a1, ...accounts] = saddle.accounts;
    chum = await deploy('CHUM', [root]);
    gov = await deploy('GovernorAlpha', [address(0), chum._address, root]);

    targets = [a1];
    values = ["0"];
    signatures = ["getBalanceOf(address)"];
    callDatas = [encodeParameters(['address'], [a1])];
    await send(chum, 'delegate', [root]);
  ...
  describe("Otherwise", () => {
    ...
    describe("and we take the balance returned by GetPriorVotes for the given sender and the proposal's start block, which may be zero,", () => {
      ...
      it("and we add that ForVotes", async () => {
        actor = accounts[1];
        await enfranchise(chum, actor, 400001);
        ...
        expect(new BigNumber(afterFors)).toEqual(new BigNumber(beforeFors).plus(maticMantissa(400001)));
      })
      ...
      it("or AgainstVotes corresponding to the caller's support flag.", async () => {
        ...
        expect(new BigNumber(afterAgainsts)).toEqual(new BigNumber(beforeAgainsts).plus(maticMantissa(400001)));
      });
    });

    describe('castVoteBySig', () => {
      const Domain = (gov) => ({
        name: 'ChumHum Governor Alpha',
      ...
      it('casts vote on behalf of the signatory', async () => {
        await enfranchise(chum, a1, 400001);
    ...
    it("receipt uses one load", async () => {
      ...
      await enfranchise(chum, actor, 400001);
      await enfranchise(chum, actor2, 400001);
      await send(gov, 'propose', [targets, values, signatures, callDatas, "do nothing"], { from: actor });
      proposalId = await call(gov, 'latestProposalIds', [actor]);
