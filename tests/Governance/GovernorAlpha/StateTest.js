...
describe('GovernorAlpha#state/1', () => {
  let chum, gov, root, acct, delay, timelock;

  beforeAll(async () => {
    await freezeTime(100);
    [root, acct, ...accounts] = accounts;
    chum = await deploy('CHUM', [root]);
    delay = maticUnsigned(2 * 24 * 60 * 60).mul(2)
    timelock = await deploy('TimelockHarness', [root, delay]);
    gov = await deploy('GovernorAlpha', [timelock._address, chum._address, root]);
    await send(timelock, "harnessSetAdmin", [gov._address])
    await send(chum, 'transfer', [acct, maticMantissa(4000000)]);
    await send(chum, 'delegate', [acct], { from: acct });
  });
  ...
  it("Canceled", async () => {
    await send(chum, 'transfer', [accounts[0], maticMantissa(4000000)]);
    await send(chum, 'delegate', [accounts[0]], { from: accounts[0] });
    await mineBlock()
    ...
    await send(chum, 'delegate', [root], { from: accounts[0] });
