...
describe('CHUMScenario', () => {
  let root, accounts;
  let chum;

  beforeEach(async () => {
    [root, ...accounts] = saddle.accounts;
    chum = await deploy('CHUMScenario', [root]);
  });
    ...
    [
      ...
      it(`with ${checkpoints} checkpoints, has ${expectedReads} reads`, async () => {
        ...
        while (remaining > 0) {
          ...
          await chum.methods.generateCheckpoints(amt, offset).send({from: root, gas: 200000000});
          remaining -= amt;
          offset += amt;
        }

        let result = await chum.methods.getPriorVotes(root, 1).send();
