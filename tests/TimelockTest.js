...
const oneWeekInSeconds = maticUnsigned(7 * 24 * 60 * 60);
const zero = maticUnsigned(0);
...
describe('Timelock', () => {
  ...
  let revertData = encodeParameters(['uint256'], [maticUnsigned(60 * 60)]);
  let eta;
  let queuedTxHash;
  ...
  beforeEach(async () => {
    ...
    blockTimestamp = maticUnsigned(100);
  ...
  describe('executeTransaction (setPendingAdmin)', () => {
    beforeEach(async () => {
      ...
      delay = maticUnsigned(configuredDelay);
