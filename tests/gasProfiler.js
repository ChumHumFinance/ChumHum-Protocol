...
async function claimChumHum(comptroller, holder) {
  return send(comptroller, 'claimChumHum', [holder], { from: holder });
}
...
describe('Gas report', () => {
  ...
  const preMintAmount = maticUnsigned(30e4);
  const mintAmount = maticUnsigned(10e4);
  ...
  describe.each([
    ...
    beforeEach(async () => {
      ...
      if (patch == 'unitroller') {
        await send(comptroller, '_setChumHumSpeed', [cToken._address, maticExp(0.05)]);
      } else {
        await send(comptroller, '_addChumHumMarkets', [[cToken].map(c => c._address)]);
        await send(comptroller, 'setChumHumSpeed', [cToken._address, maticExp(0.05)]);
      }
      await send(comptroller.chum, 'transfer', [comptroller._address, maticUnsigned(50e18)], {from: root});
    });

    it(`${patch} second mint with chum accrued`, async () => {
      await mint(cToken, minter, mintAmount, exchangeRate);

      await fastForwardPatch(patch, comptroller, 10);

      console.log('CHUM balance before mint', (await chumBalance(comptroller, minter)).toString());
      console.log('CHUM accrued before mint', (await chumhumAccrued(comptroller, minter)).toString());
      const mint2Receipt = await mint(cToken, minter, mintAmount, exchangeRate);
      console.log('CHUM balance after mint', (await chumBalance(comptroller, minter)).toString());
      console.log('CHUM accrued after mint', (await chumhumAccrued(comptroller, minter)).toString());
      recordGasCost(mint2Receipt.gasUsed, `${patch} second mint with chum accrued`, filename);
    });

    it(`${patch} claim chum`, async () => {
      await mint(cToken, minter, mintAmount, exchangeRate);

      await fastForwardPatch(patch, comptroller, 10);

      console.log('CHUM balance before claim', (await chumBalance(comptroller, minter)).toString());
      console.log('CHUM accrued before claim', (await chumhumAccrued(comptroller, minter)).toString());
      const claimReceipt = await claimChumHum(comptroller, minter);
      console.log('CHUM balance after claim', (await chumBalance(comptroller, minter)).toString());
      console.log('CHUM accrued after claim', (await chumhumAccrued(comptroller, minter)).toString());
      recordGasCost(claimReceipt.gasUsed, `${patch} claim chum`, filename);
    });
  });
});
