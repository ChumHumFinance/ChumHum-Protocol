...
export async function buildGovernor(
  ...
  const fetchers = [
    new Fetcher<
      { name: StringV, timelock: AddressV, chum: AddressV, guardian: AddressV },
      GovernorData
    >(
      `
      #### GovernorAlpha

      * "Governor Deploy Alpha name:<String> timelock:<Address> chum:<Address> guardian:<Address>" - Deploys ChumHum Governor Alpha
        * E.g. "Governor Deploy Alpha GovernorAlpha (Address Timelock) (Address CHUM) Guardian"
    `,
      "Alpha",
      [
        new Arg("name", getStringV),
        new Arg("timelock", getAddressV),
        new Arg("chum", getAddressV),
        new Arg("guardian", getAddressV)
      ],
      async (world, { name, timelock, chum, guardian }) => {
        return {
          invokation: await GovernorAlphaContract.deploy<Governor>(
            world,
            from,
            [timelock.val, chum.val, guardian.val]
          ),
          name: name.val,
    ...
    >(
      ...
      * "Governor Deploy AlphaHarness name:<String> timelock:<Address> chum:<Address> guardian:<Address>" - Deploys ChumHum Governor Alpha with a mocked voting period
        * E.g. "Governor Deploy AlphaHarness GovernorAlphaHarness (Address Timelock) (Address CHUM) Guardian"
    `,
      "AlphaHarness",
      [
        new Arg("name", getStringV),
