...
async function getChumHumMarkets(world: World, comptroller: Comptroller): Promise<ListV> {
  let mkts = await comptroller.methods.getChumHumMarkets().call();
...
async function checkIsChumHum(world: World, comptroller: Comptroller, cToken: CToken): Promise<BoolV> {
  let {0: isListed, 1: _collateralFactorMantissa, 2: isChumHum} = await comptroller.methods.markets(cToken._address).call();
  return new BoolV(isChumHum);
}

async function mintedBUMs(world: World, comptroller: Comptroller, user: string): Promise<NumberV> {
  return new NumberV(await comptroller.methods.mintedBUMs(user).call());
}
...
export function comptrollerFetchers() {
  return [
    ...
    new Fetcher<{comptroller: Comptroller, cToken: CToken}, BoolV>(`
        ...
        * "Comptroller CheckIsChumHum <CToken>" - Returns true if market is listed, false otherwise.
          * E.g. "Comptroller CheckIsChumHum cZRX"
      `,
      "CheckIsChumHum",
      [
        new Arg("comptroller", getComptroller, {implicit: true}),
      ...
      (world, {comptroller, cToken}) => checkIsChumHum(world, comptroller, cToken)
    ),
    ...
    new Fetcher<{comptroller: Comptroller}, ListV>(`
      ...
      * "GetChumHumMarkets" - Returns an array of the currently enabled ChumHum markets. To use the auto-gen array getter chumhumMarkets(uint), use ChumHumMarkets
      * E.g. "Comptroller GetChumHumMarkets"
      `,
      "GetChumHumMarkets",
      [new Arg("comptroller", getComptroller, {implicit: true})],
      async(world, {comptroller}) => await getChumHumMarkets(world, comptroller)
     ),

    new Fetcher<{comptroller: Comptroller}, NumberV>(`
      #### ChumHumRate

      * "ChumHumRate" - Returns the current chum rate.
      * E.g. "Comptroller ChumHumRate"
      `,
      "ChumHumRate",
      [new Arg("comptroller", getComptroller, {implicit: true})],
      async(world, {comptroller}) => new NumberV(await comptroller.methods.chumhumRate().call())
    ),
    ...
    new Fetcher<{comptroller: Comptroller, signature: StringV, callArgs: StringV[]}, NumberV>(`
        ...
        * "CallNum signature:<String> ...callArgs<CoreValue>" - Simple direct call method
          * E.g. "Comptroller CallNum \"chumhumSpeeds(address)\" (Address Coburn)"
      `,
      "CallNum",
      [
        new Arg("comptroller", getComptroller, {implicit: true}),
    ...
    new Fetcher<{comptroller: Comptroller, CToken: CToken, key: StringV}, NumberV>(`
        #### ChumHumSupplyState(address)

        * "Comptroller ChumHumBorrowState cZRX "index"
      `,
      "ChumHumSupplyState",
      [
        new Arg("comptroller", getComptroller, {implicit: true}),
      ...
      async (world, {comptroller, CToken, key}) => {
        const result = await comptroller.methods.chumhumSupplyState(CToken._address).call();
        return new NumberV(result[key.val]);
      }
    ),
    new Fetcher<{comptroller: Comptroller, CToken: CToken, key: StringV}, NumberV>(`
        #### ChumHumBorrowState(address)
      ...
      async (world, {comptroller, CToken, key}) => {
        const result = await comptroller.methods.chumhumBorrowState(CToken._address).call();
        return new NumberV(result[key.val]);
      }
    ),
    new Fetcher<{comptroller: Comptroller, account: AddressV, key: StringV}, NumberV>(`
        #### ChumHumAccrued(address)

        * "Comptroller ChumHumAccrued Coburn
      `,
      "ChumHumAccrued",
      [
        new Arg("comptroller", getComptroller, {implicit: true}),
      ...
      async (world, {comptroller,account}) => {
        const result = await comptroller.methods.chumhumAccrued(account.val).call();
        return new NumberV(result);
      }
    ),
    new Fetcher<{comptroller: Comptroller, CToken: CToken, account: AddressV}, NumberV>(`
        #### chumhumSupplierIndex

        * "Comptroller ChumHumSupplierIndex cZRX Coburn
      `,
      "ChumHumSupplierIndex",
      [
        new Arg("comptroller", getComptroller, {implicit: true}),
      ...
      async (world, {comptroller, CToken, account}) => {
        return new NumberV(await comptroller.methods.chumhumSupplierIndex(CToken._address, account.val).call());
      }
    ),
    new Fetcher<{comptroller: Comptroller, CToken: CToken, account: AddressV}, NumberV>(`
        #### ChumHumBorrowerIndex

        * "Comptroller ChumHumBorrowerIndex cZRX Coburn
      `,
      "ChumHumBorrowerIndex",
      [
        new Arg("comptroller", getComptroller, {implicit: true}),
      ...
      async (world, {comptroller, CToken, account}) => {
        return new NumberV(await comptroller.methods.chumhumBorrowerIndex(CToken._address, account.val).call());
      }
    ),
    new Fetcher<{comptroller: Comptroller, CToken: CToken}, NumberV>(`
        #### ChumHumSpeed

        * "Comptroller ChumHumSpeed cZRX
      `,
      "ChumHumSpeed",
      [
        new Arg("comptroller", getComptroller, {implicit: true}),
      ...
      async (world, {comptroller, CToken}) => {
        return new NumberV(await comptroller.methods.chumhumSpeeds(CToken._address).call());
      }
    ),
    new Fetcher<{ comptroller: Comptroller, address: AddressV }, NumberV>(`
        #### MintedBUM

        * "Comptroller MintedBUM <User>" - Returns a user's minted bum amount
          * E.g. "Comptroller MintedBUM Geoff"
      `,
      "MintedBUM",
      [
        new Arg("comptroller", getComptroller, {implicit: true}),
      ...
      (world, { comptroller, address }) => mintedBUMs(world, comptroller, address.val),
    ),
    new Fetcher<{comptroller: Comptroller}, AddressV>(`
        #### BorrowCapGuardian
