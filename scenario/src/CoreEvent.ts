...
import { bumcontrollerCommands, processBUMControllerEvent } from './Event/BUMControllerEvent';
import { processUnitrollerEvent, unitrollerCommands } from './Event/UnitrollerEvent';
import { processBUMUnitrollerEvent, bumunitrollerCommands } from './Event/BUMUnitrollerEvent';
import { processBUMVaultProxyEvent, bumvaultproxyCommands } from './Event/BUMVaultProxyEvent';
import { processBUMVaultEvent, bumvaultCommands } from './Event/BUMVaultEvent';
import { comptrollerImplCommands, processComptrollerImplEvent } from './Event/ComptrollerImplEvent';
import { bumcontrollerImplCommands, processBUMControllerImplEvent } from './Event/BUMControllerImplEvent';
import { bumvaultImplCommands, processBUMVaultImplEvent } from './Event/BUMVaultImplEvent';
...
import { chumCommands, processCHUMEvent } from './Event/CHUMEvent';
import { bumCommands, processBUMEvent } from './Event/BUMEvent';
...
import { ChumHumLens } from './Contract/ChumHumLens';
...
async function sendMATIC(world: World, from: string, to: string, amount: encodedNumber): Promise<World> {
...
export const commands: (View<any> | ((world: World) => Promise<View<any>>))[] = [
  ...
  new View<{ networkVal: StringV; }>(
    ...
    async (world, { networkVal }) => {
      ...
      if (world.basePath && (network === 'mainnet' || network === 'kovan' || network === 'goerli' || network === 'rinkeby' || network == 'ropsten' || network == 'bscmainnet' || network == 'matic' || network == 'bsctestnet' )) {
  ...
  new Command<{ blockNumber: NumberV, event: EventV }>(
    `
      ...
      * "Block 10 (...event)" - Set block to block N and run event
        * E.g. "Block 10 (CHUM Deploy Admin)"
    `,
    'Block',
    [
      new Arg('blockNumber', getNumberV),
  ...
  new Command<{ event: EventV }>(
    `
      ...
      * "Trx ...trxEvent" - Handles event to set details of next transaction
        * E.g. "Trx Value 1.0e18 (CToken cMatic Mint 1.0e18)"
    `,
    'Trx',
  ...
  new Command<{ address: AddressV; amount: NumberV }>(
    `
      ...
      * "Send <Address> <Amount>" - Sends a given amount of matic to given address
        * E.g. "Send cMATIC 0.5e18"
    `,
    'Send',
    [new Arg('address', getAddressV), new Arg('amount', getNumberV)],
    (world, from, { address, amount }) => sendMATIC(world, from, address.val, amount.encode())
  ),
  ...
  new Command<{ event: EventV }>(
    `
      ...
      * "BUMUnitroller ...event" - Runs given BUMUnitroller event
        * E.g. "BUMUnitroller SetPendingImpl MyBUMControllerImpl"
    `,
    'BUMUnitroller',
    [new Arg('event', getEventV, { variadic: true })],
    (world, from, { event }) => processBUMUnitrollerEvent(world, event.val, from),
    { subExpressions: bumunitrollerCommands() }
  ),
  ...
  new Command<{ event: EventV }>(
    `
      ...
      * "BUMController ...event" - Runs given BUMController event
        * E.g. "BUMController mint 0.5"
    `,
    'BUMController',
    [new Arg('event', getEventV, { variadic: true })],
    (world, from, { event }) => processBUMControllerEvent(world, event.val, from),
    { subExpressions: bumcontrollerCommands() }
  ),
  ...
  new Command<{ event: EventV }>(
    `
      ...
      * "BUMControllerImpl ...event" - Runs given BUMControllerImpl event
        * E.g. "BUMControllerImpl MyImpl Become"
    `,
    'BUMControllerImpl',
    [new Arg('event', getEventV, { variadic: true })],
    (world, from, { event }) => processBUMControllerImplEvent(world, event.val, from),
    { subExpressions: bumcontrollerImplCommands() }
  ),
  new Command<{ event: EventV }>(
    `
      #### BUMVaultProxy

      * "BUMVaultProxy ...event" - Runs given BUMVaultProxy event
        * E.g. "BUMVaultProxy SetPendingImpl MyComptrollerImpl"
    `,
    'BUMVaultProxy',
    [new Arg('event', getEventV, { variadic: true })],
    (world, from, { event }) => processBUMVaultProxyEvent(world, event.val, from),
    { subExpressions: bumvaultproxyCommands() }
  ),
  new Command<{ event: EventV }>(
    `
      #### BUMVault

      * "BUMVault ...event" - Runs given BUMVault event
        * E.g. "BUMVault mint 0.5"
    `,
    'BUMVault',
    [new Arg('event', getEventV, { variadic: true })],
    (world, from, { event }) => processBUMVaultEvent(world, event.val, from),
    { subExpressions: bumvaultCommands() }
  ),
  new Command<{ event: EventV }>(
    `
      #### BUMVaultImpl

      * "BUMVaultImpl ...event" - Runs given BUMVaultImpl event
        * E.g. "BUMVaultImpl MyImpl Become"
    `,
    'BUMVaultImpl',
    [new Arg('event', getEventV, { variadic: true })],
    (world, from, { event }) => processBUMVaultImplEvent(world, event.val, from),
    { subExpressions: bumvaultImplCommands() }
  ),
  new Command<{ event: EventV }>(
    `
      #### CToken
  ...
  new Command<{ event: EventV }>(
    `
      ...
      * E.g. "PriceOracleProxy Deploy (Unitroller Address) (PriceOracle Address) (CToken cMATIC Address)"
    `,
    'PriceOracleProxy',
    [new Arg('event', getEventV, { variadic: true })],
    (world, from, { event }) => {
  ...
  new Command<{ event: EventV }>(
    `
      ...
      * E.g. "Maximillion Deploy (CToken cMATIC Address)"
    `,
    'Maximillion',
    [new Arg('event', getEventV, { variadic: true })],
    (world, from, { event }) => {
  ...
  new Command<{ event: EventV }>(
    `
      ...
      * "CHUM ...event" - Runs given chum event
      * E.g. "CHUM Deploy"
    `,
    'CHUM',
    [new Arg('event', getEventV, { variadic: true })],
    (world, from, { event }) => {
      return processCHUMEvent(world, event.val, from);
    },
    { subExpressions: chumCommands() }
  ),
  ...
  new Command<{ event: EventV }>(
    `
      ...
      * "BUM ...event" - Runs given chum event
      * E.g. "BUM Deploy"
    `,
    'BUM',
    [new Arg('event', getEventV, { variadic: true })],
    (world, from, { event }) => {
      return processBUMEvent(world, event.val, from);
    },
    { subExpressions: bumCommands() }
  ),
  ...
  buildContractEvent<ChumHumLens>("ChumHumLens", false),
