...
import { bumcontrollerFetchers, getBUMControllerValue } from './Value/BUMControllerValue';
import { bumvaultFetchers, getBUMVaultValue } from './Value/BUMVaultValue';
import { bumvaultImplFetchers, getBUMVaultImplValue } from './Value/BUMVaultImplValue';
import { bumcontrollerImplFetchers, getBUMControllerImplValue } from './Value/BUMControllerImplValue';
import { getUnitrollerValue, unitrollerFetchers } from './Value/UnitrollerValue';
import { getBUMVaultProxyValue, bumvaultproxyFetchers } from './Value/BUMVaultProxyValue';
...
import { getCHUMValue, chumFetchers } from './Value/CHUMValue';
import { getBUMValue, bumFetchers } from './Value/BUMValue';
...
async function getMATICBalance(world: World, address: string): Promise<NumberV> {
...
const fetchers = [
    ...
    async (world, { addr, slot, key, nestedKey, valType }) => {
      ...
      switch (valType.val) {
        case 'marketStruct':
          ...
          let isChumHumStr = await world.web3.eth.getStorageAt(addr.val, isCompKey);

          return new ListV([
            new BoolV(isListed),
            new ExpNumberV(collateralFactor.toString(), 1e18),
            new BoolV(areEqual(userInMarket, 1)),
            new BoolV(areEqual(isChumHumStr, 1))
          ]);
        default:
          return new NothingV();
      }
  ...
  new Fetcher<{ address: AddressV }, Value>(
    `
      ...
      * "MATICBalance <Address>" - Returns given address' matic balance.
    `,
    'MATICBalance',
    [new Arg('address', getAddressV)],
    (world, { address }) => getMATICBalance(world, address.val)
  ),
  ...
  new Fetcher<{ res: Value }, Value>(
    `
      ...
      * "BUMVaultProxy ...bumvaultproxyArgs" - Returns bumvaultproxy value
    `,
    'BUMVaultProxy',
    [new Arg('res', getBUMVaultProxyValue, { variadic: true })],
    async (world, { res }) => res,
    { subExpressions: bumvaultproxyFetchers() }
  ),
  new Fetcher<{ res: Value }, Value>(
    `
      #### Comptroller
  ...
  new Fetcher<{ res: Value }, Value>(
    `
      ...
      * "BUMController ...bumcontrollerArgs" - Returns bumcontroller value
    `,
    'BUMController',
    [new Arg('res', getBUMControllerValue, { variadic: true })],
    async (world, { res }) => res,
    { subExpressions: bumcontrollerFetchers() }
  ),
  new Fetcher<{ res: Value }, Value>(
    `
      #### BUMControllerImpl

      * "BUMControllerImpl ...bumcontrollerImplArgs" - Returns bumcontroller implementation value
    `,
    'BUMControllerImpl',
    [new Arg('res', getBUMControllerImplValue, { variadic: true })],
    async (world, { res }) => res,
    { subExpressions: bumcontrollerImplFetchers() }
  ),
  new Fetcher<{ res: Value }, Value>(
    `
      #### BUMVault

      * "BUMVault ...bumvaultArgs" - Returns bumvault value
    `,
    'BUMVault',
    [new Arg('res', getBUMVaultValue, { variadic: true })],
    async (world, { res }) => res,
    { subExpressions: bumvaultFetchers() }
  ),
  new Fetcher<{ res: Value }, Value>(
    `
      #### BUMVaultImpl

      * "BUMVaultImpl ...bumcontrollerImplArgs" - Returns bumvault implementation value
    `,
    'BUMVaultImpl',
    [new Arg('res', getBUMVaultImplValue, { variadic: true })],
    async (world, { res }) => res,
    { subExpressions: bumvaultImplFetchers() }
  ),
  new Fetcher<{ res: Value }, Value>(
    `
      #### CToken
  ...
  new Fetcher<{ res: Value }, Value>(
    `
      ...
      * "CHUM ...chumhumArgs" - Returns CHUM value
    `,
    'CHUM',
    [new Arg('res', getCHUMValue, { variadic: true })],
    async (world, { res }) => res,
    { subExpressions: chumFetchers() }
  ),
  new Fetcher<{ res: Value }, Value>(
    `
      #### BUM

      * "BUM ...chumhumArgs" - Returns BUM value
    `,
    'BUM',
    [new Arg('res', getBUMValue, { variadic: true })],
    async (world, { res }) => res,
    { subExpressions: bumFetchers() }
  ),
  new Fetcher<{ res: Value }, Value>(
    `
      #### Governor
...
let contractFetchers = [
  ...
  { contract: "ChumHumLens", implicit: false },
