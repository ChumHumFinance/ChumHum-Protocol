...
spec/certora/CHUM/search.cvl:
	$(CERTORA_RUN) \
	spec/certora/contracts/CHUMCertora.sol \
	--settings -b=4,-graphDrawLimit=0,-assumeUnwindCond,-depth=100 \
	--solc_args "'--evm-version istanbul'" \
	--verify \
	 CHUMCertora:$@

spec/certora/CHUM/transfer.cvl:
	$(CERTORA_RUN) \
	spec/certora/contracts/CHUMCertora.sol \
...
spec/certora/Governor/%.cvl:
	$(CERTORA_RUN) \
	 ...
	 GovernorAlphaCertora:chum=CHUMCertora \
	--verify \
...
spec/certora/Maximillion/%.cvl:
	$(CERTORA_RUN) \
	 ...
	 spec/certora/contracts/CMATICCertora.sol \
	--link \
	 MaximillionCertora:cMatic=CMATICCertora \
	--verify \
	 MaximillionCertora:$@
